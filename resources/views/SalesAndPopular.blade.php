...
<div class="products">
    <div class="top-line">
        <p class="sale">Популярні товари</p>
        <p class="sale"><img src="{{ asset('images/line.png') }}" width="95%"></p>
    </div><br>
    <div class="product-roster">
        @for($i = 0; $i < 3; $i++)
                @for($j = 0; $j < 3; $j++)
                    <?php
                    $rand_tovar = random_int(0, count($popular)-1);
                    ?>
                        <div class="product-item">
                            <div class="product-img">
                                <a href="{{$popular[$rand_tovar]->id}}">
                                    <img src="images/{{$popular[$rand_tovar]->img}}">
                                </a>
                            </div>
                            <div class="product-list">
                                <h3>{{$popular[$rand_tovar]->name}}</h3>
                                <span class="price">₴ {{$popular[$rand_tovar]->price}}</span>
                                <div class="actions">
                                    @if($popular[$rand_tovar]->visible)
                                        <div class="add-to-cart">
                                            <a href="add-to-cart/{{$popular[$rand_tovar]->id}}" class="cart-button">В кошик</a>
                                        </div>
                                    @else
                                        <h3>Доступність: немає</h3>
                                    @endif
